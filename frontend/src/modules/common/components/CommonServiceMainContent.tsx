  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center justify-between w-40 px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2  gap-5">
        {role === "agency" ? (
          (mainContentData as CommonServiceCardProps[] | undefined)?.map(
            (item, index) => <CommonServiceCard key={index} data={item} />
          )
        ) : (
          (mainContentData as CommonProjectCardProps[] | undefined)?.map(
            (item, index) => <CommonProjectCard key={index} data={item} />
          )
        )}
      </div>
