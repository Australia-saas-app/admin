...
export class JwtService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.accessTokenTtl = parseInt(this.configService.get('ACCESS_TOKEN_TTL', '900'), 10); // 15 minutes
  ...
  verifyAccessToken(token: string): AccessTokenPayload | null {
    try {
      ...
      // Try without issuer validation to debug
      try {
        const payload = verify(token, this.publicKey, {
          algorithms: ['RS256'],
        }) as AccessTokenPayload;
        console.log("Token verified without issuer check, payload:", payload);
        return payload;
      } catch {
        return null;
      }
