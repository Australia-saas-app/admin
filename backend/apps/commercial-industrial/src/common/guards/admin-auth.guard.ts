  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    try {
      ...
      try {
        ...
        const payloadPart = Buffer.from(parts[1].replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf-8');
      ...
      try {
        ...
        if (!tokenIssuer.includes('sso') && tokenIssuer !== this.issuer) {
          ...
          if (!allowedIssuers.includes(tokenIssuer)) {
            throw new UnauthorizedException(`Token issuer mismatch. Expected SSO issuer but got '${tokenIssuer}'`);
          }
        }
      ...
      throw new UnauthorizedException(`Invalid or expired admin token: ${errorMessage}`);
    }
  }
}
