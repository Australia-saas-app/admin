...
const CommonServiceCard = ({ data }: { data: CommonServiceCardProps }) => {
  return (
    <div className="card-lift space-y-1 rounded-2xl border border-border bg-card p-4 shadow-sm">
      <h1 className="text-primary text-xl font-semibold">{data.title}</h1>
      <div className="flex flex-wrap gap-[3px] text-foreground">
        <h2> {data.category},</h2>
        <h2>{data.subCategory.join(", ")}</h2>
      </div>
      <p className="text-muted-foreground font-bold">${data.price}</p>
      <p className="text-sm text-muted-foreground">{data?.projectDescription?.slice(0, 100)}...</p>
      ...
      <div className="flex justify-between flex-wrap space-y-1">
        <div className="flex  items-center gap-2">
          ...
          <span className="text-muted-foreground border border-gray-500 px-2 py-[1px] text-sm rounded-3xl">
            {data?.priorityLevel}
          </span>
        </div>
