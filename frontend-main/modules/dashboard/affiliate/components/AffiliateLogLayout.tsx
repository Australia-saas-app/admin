...
import { AFFILIATE_LOG_ENTRIES } from "../data/affiliate-demo-data"

export default function AffiliateLogLayout() {
  const { t } = useLocale()
  const { demoOrEmpty } = useIsDemoAccount()
  const entries = demoOrEmpty(AFFILIATE_LOG_ENTRIES, [] as typeof AFFILIATE_LOG_ENTRIES)
  const p = t.affiliatePages.affiliateLog
    ...
    <div className="rounded-xl border border-gray-100 bg-white p-6 md:p-8 shadow-sm">
      ...
      <div className="divide-y divide-gray-100">
        {entries.length === 0 ? (
          ...
          <div key={`${entry.action}-${entry.date}`} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 py-3.5 text-sm">
