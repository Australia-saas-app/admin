 ...
 */
export async function serverFetch(input: string, options?: FetchOptions) {
...
export async function serverFetchJson(input: string, options?: FetchOptions) {
  const res = await serverFetch(input, options);
  if (!res.ok) {
