import { loginUser, registerUser, requestPasswordReset, confirmPasswordReset, logout as apiLogout, demoLogin } from "@/src/server/AuthService"
...
const getDashboardRedirect = (role?: string, userId?: string) => {
    ...
    const accountType = accountTypeFromRole(role)
    if (userId?.startsWith("registered-") && !isProfileComplete(userId)) {
        return completeProfilePath(accountType)
    }

    if (normalized === "AFFILIATE") return "/affiliate/dashboard";
    if (normalized === "SELLER" || normalized === "BUSINESS") return "/business/dashboard";
    if (normalized === "BUYER" || normalized === "USER") return "/user/dashboard";
    return ROUTES.home;
};
...
export const useDemoLogin = () => {
    ...
    return useMutation<AuthResponse, Error, DemoAccountType>({
        mutationKey: ["DEMO_LOGIN"],
        mutationFn: async (accountType) => await demoLogin(accountType),
        onSuccess: async (data, accountType) => {
            toast.success(`Logged in as ${DEMO_ACCOUNTS[accountType].label}`);
            activityLog.record("USER_LOGIN", { role: data?.data?.user?.role, demo: true });
            await refreshUser();
            const redirectTo = DEMO_ACCOUNTS[accountType].redirect;
            router.refresh();
            if (typeof window !== "undefined") {
                window.location.assign(redirectTo);
                return;
        ...
        onError: (error) => {
            toast.error(parseApiError(error, 'Demo login failed'));
        }
    })
}
